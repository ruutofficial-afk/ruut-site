<?php
/**
 * Plugin Name: RUUT Custom Backend
 * Description: Unified WordPress, WooCommerce Customizations & Headless REST API Handler (Safe Edition)
 * Version: 1.0.0
 * Author: RUUT
 * 
 * This file combines:
...
if (!function_exists('ruut_enqueue_my_account_scripts')) {
    ...
    function ruut_enqueue_my_account_scripts() {
        if ( is_account_page() ) {
            if ( ! is_user_logged_in() ) {
                wp_enqueue_script( 'ruut-login-otp', plugins_url( 'assets/checkout.js', __FILE__ ), array('jquery'), '1.1', true );
                wp_localize_script( 'ruut-login-otp', 'ruut_ajax', array(
                    'url'   => admin_url( 'admin-ajax.php' ),
                    'nonce' => wp_create_nonce( 'ruut_otp_nonce' )
                ) );
            }
            wp_enqueue_script( 'ruut-account-scripts', plugins_url( 'assets/checkout.js', __FILE__ ), array('jquery'), '1.1', true );
            wp_localize_script( 'ruut-account-scripts', 'ruut_account_ajax', array(
                'ajax_url' => admin_url( 'admin-ajax.php' )
            ));
            wp_enqueue_style( 'ruut-account-styles', plugins_url( 'assets/checkout.css', __FILE__ ) );
        }
    }
}
